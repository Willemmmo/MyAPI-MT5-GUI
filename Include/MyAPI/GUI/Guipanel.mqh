...
#define GUIPANEL_MQH
#include "GUIBase.mqh"
#include "GUITabBar.mqh"
#include "GUIContent.mqh"
...
public:
                  CGUIPanel(long chartID, string prefix, string title,
     ...
     {
      m_tabBar  = new CGUITabBar(chartID, prefix);
      m_content = new CGUIContent(chartID, prefix);
     }
   ...
   CGUIContent   *Content() { return m_content; }
   ...
   virtual bool   OnMouseDown(int mx, int my)
     {
      ...
      //--- History header click (sort column)
      if(!m_minimized && m_content != NULL && m_content.OnHistoryClick(mx, my))
